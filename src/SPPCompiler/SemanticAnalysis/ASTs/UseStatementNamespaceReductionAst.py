...
class UseStatementNamespaceReductionAst(Ast, Stage2_SymbolGenerator, Stage3_SupScopeLoader, Stage4_SemanticAnalyser):
    ...
    def _convert_to_type_aliases(self, scope_manager: ScopeManager) -> None:
        from SPPCompiler.SemanticAnalysis import UseStatementNamespaceReductionTypesMultipleAst, TypeAst
        from SPPCompiler.SemanticAnalysis import UseStatementNamespaceReductionTypesSingleAst, GenericArgumentNamedAst

        types = {}

        def merge_layers(ast: UseStatementNamespaceReductionBodyAst, current_ns: Seq[IdentifierAst]) -> None:
            if isinstance(ast, UseStatementNamespaceReductionTypesSingleAst):
                # Save the original length of the namespace, and add the next nested namespace.
                original_ns_len = current_ns.length
                current_ns += ast.namespace

                # Create a new type with the current namespace and the types from the import.
                new_type = TypeAst(-1, namespace=current_ns.copy(), types=ast.types)
                ...
                type_alias = TypeAst(-1, Seq(), ast.alias.type.types.copy()) if ast.alias else None
                types[new_type] = type_alias

            elif isinstance(ast, UseStatementNamespaceReductionTypesMultipleAst):
                # Save the original length of the namespace, and add the next nested namespace.
                original_ns_len = current_ns.length
                current_ns += ast.namespace

                # For each type in the import, recursively combine the layers.
                for inner_type in ast.types:
        ...
        for type, alias in types.items():
            new_ast = UseStatementTypeAliasAst.from_types(IdentifierAst.from_type((alias or type).types[-1]), None, type)
