...
@dataclass
class UseStatementNamespaceReductionAst:  # (Ast, Stage0_PreProcessor, Stage1_SymbolGenerator, Stage2_TypeAliaser, Stage3_SupScopeLoader, Stage4_SemanticAnalyser):
    ...

    # body: UseStatementNamespaceReductionBodyAst
    #
    # _new_asts_0: Seq[Tuple[TypeAst, Optional[UseStatementTypeAliasAst]]] = field(default_factory=Seq, init=False, repr=False)
    # _new_asts_1: Seq[UseStatementTypeAliasAst] = field(default_factory=Seq, init=False, repr=False)
    ...
    #
    # def _convert_to_type_aliases_stage_0(self, scope_manager: ScopeManager) -> None:
    #     from SPPCompiler.SemanticAnalysis import UseStatementNamespaceReductionTypesMultipleAst, TypeAst, IdentifierAst
    #     from SPPCompiler.SemanticAnalysis import UseStatementNamespaceReductionTypesSingleAst
    #
    #     types = {}
    #
    ...
    #         self._new_asts_0.append((new_type, alias))
    #
    # def _convert_to_type_aliases_stage_1(self, scope_manager: ScopeManager) -> None:
    ...
    #     from SPPCompiler.SemanticAnalysis import GenericArgumentGroupAst, UseStatementTypeAliasAst, IdentifierAst
    #
    #     GenericArgumentCTor = {
    ...
    #
    #     for new_type, alias in self._new_asts_0:
    ...
    #
    #         self._new_asts_1.append(new_ast)
    ...
    #     self._convert_to_type_aliases_stage_0(scope_manager)
    #     self._new_asts_1.for_each(lambda ast: ast.generate_symbols(scope_manager))
    #
    # def alias_types(self, scope_manager: ScopeManager) -> None:
    #     # Convert the import aliases to type aliases: "use std::Str" => "use std::Str as Str".
    #     self._convert_to_type_aliases_stage_1(scope_manager)
    #     self._new_asts_1.for_each(lambda ast: ast.alias_types(scope_manager))
    #
    # def load_sup_scopes(self, scope_manager: ScopeManager) -> None:
    #     # Load the sup scopes of the new type aliases.
    #     self._new_asts_1.for_each(lambda ast: ast.load_sup_scopes(scope_manager))
    #
    # def inject_sup_scopes(self, scope_manager: ScopeManager) -> None:
    #     # Inject the sup scopes of the new type aliases.
    #     self._new_asts_1.for_each(lambda ast: ast.inject_sup_scopes(scope_manager))
    #
    ...
    #         self._convert_to_type_aliases_stage_0(scope_manager)
    #         self._convert_to_type_aliases_stage_1(scope_manager)
    #     self._new_asts_1.for_each(lambda ast: ast.analyse_semantics(scope_manager, **kwargs))
