...
class TestGenExpressionAst(CustomTestCase):
    ...
    def test_invalid_gen_expression_convention_mismatch_1a(self):
        """
        cor foo() -> std::generator::Gen[std::bignum::bigint::BigInt] {
            gen &1
        }
        """
    ...
    def test_invalid_gen_expression_convention_mismatch_3a(self):
        """
        cor foo() -> std::generator::Gen[&mut std::bignum::bigint::BigInt] {
            gen 1
        }
        """
    ...
    def test_invalid_gen_expression_unroll(self):
        ...
        cor bar() -> std::generator::Gen[&mut std::bignum::bigint::BigInt] {
            gen &mut 0
            gen with foo()
            gen &mut 4
        }
    ...
    def test_invalid_gen_expression_received_type_mismatch_1a(self):
        """
        cor foo() -> std::generator::Gen[std::bignum::bigint::BigInt, std::boolean::Bool] {
            gen &1
        }

        fun foo(y: std::bignum::bigint::BigInt) -> std::void::Void {
            let g = foo()
    ...
    def test_invalid_gen_expression_received_type_mismatch_1b(self):
        ...
        fun foo(y: &mut std::bignum::bigint::BigInt) -> std::void::Void {
            let g = foo()
            let mut x = g.res(false)
            x = y
        }
